...
#define SPIDER_MINS { -12.0, -12.0, 0.0 }
#define SPIDER_MAXS { 12.0, 12.0, 24.0 }

new const szClassNameEnt[ ] = "sb_spider";
new const szModelEnt[ ] = "models/asylumgamers/spookypumpkin.mdl";
...
public plugin_init( ) {
	...
	register_think( szClassNameEnt, "fw_Think" );
...
public fw_Touch( toucher, touched ) {
	...
	client_print_color( toucher, print_team_default, "%s Agarraste una calabaza.", szPrefix );
	g_iSpider[ toucher ]++;
...
public fw_Think( ent ) {
	...
	entity_set_float( ent, EV_FL_nextthink, halflife_time( ) + 1.2 );
}
...
public fn_CreateSpider( iOrigin[ 3 ] ) {
	...
	entity_set_vector( i_Ent, EV_VEC_origin, fOrigin );
	entity_set_string( i_Ent, EV_SZ_classname, szClassNameEnt );
	entity_set_string( i_Ent, EV_SZ_target, "pumpkin" );
	entity_set_int( i_Ent, EV_INT_solid, SOLID_BBOX );
	entity_set_int( i_Ent, EV_INT_movetype, MOVETYPE_PUSHSTEP );
	entity_set_model( i_Ent, szModelEnt );
	entity_set_size( i_Ent, Float:SPIDER_MINS, Float:SPIDER_MAXS );
	entity_set_int( i_Ent, EV_INT_renderfx, kRenderFxGlowShell );
	entity_set_vector( i_Ent, EV_VEC_rendercolor, Float:{ 146.0, 110.0, 174.0 } );
	entity_set_float( i_Ent, EV_FL_gravity, 1.0 );
	entity_set_float( i_Ent, EV_FL_friction, 0.1 );
	entity_set_float( i_Ent, EV_FL_nextthink, halflife_time( ) + 1.2 );
	drop_to_floor( i_Ent );
}
	...
	if( SQL_Execute( iQuery ) ) {
		while( SQL_MoreResults( iQuery ) ) {
			...
			if( equal( g_szPlayerName[ id ], szName ) ) {
				client_print_color( id, print_team_default, "%s Tu rank es^4 %d^1 con^4 %d^1 calabazas.", szPrefix, iCount, g_iSpider[ id ] );
				break;
			}
	...
	switch( Data[ 1 ] ) {
		...
		case TOP15_DATA: {
			if( SQL_NumResults( Query ) ) {
				...
				len += format( szBuffer[ len ], charsmax( szBuffer ) - len, "<tr align=center bgcolor=#52697B><th width=5%%> # <th width=22%% align=left> Nombre <th width=10%%> Calabazas" );
